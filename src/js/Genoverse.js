...
const Genoverse = Base.extend(
  {
    ...
    constructor: function (config = {}) {
      ...
      jQuery
        .when(this.loadCSS(), this.loadGenome(), this.loadPlugins())
        .always(() => {
    ...
    loadCSS: function () {
      ...
      return jQuery.when(
        Genoverse.importCSS
          ? import("../css/genoverse.css")
          : jQuery.Deferred().resolve(),
        Genoverse.importFontAwesome
    ...
    loadPlugins: function (plugins) {
      ...
      const pluginsByName = (plugins || this.plugins)
        .map((plugin) =>
      ...
      const initializePlugin = (plugin) => {
        ...
        if (requires) {
          Object.keys(requires).forEach((pluginName) => {
        ...
        this.container.addClass(
          `gv-${plugin.name.replace(/([A-Z])/g, "-$1").toLowerCase()}-plugin`,
        );
        this.loadedPlugins[plugin.name] = true;
      };
      ...
      const pluginImports = Object.keys(pluginsByName).reduce(
        (ready, pluginName) =>
          ready.then(() =>
            import(`./plugins/${pluginName}`).then((imported) =>
              initializePlugin({
                name: pluginName,
                conf: pluginsByName[pluginName].conf,
    ...
    init: function () {
      ...
      this.useHash =
        typeof this.useHash === "boolean"
          ? this.useHash
          : typeof window.history.pushState !== "function";
      this.textWidth = document
      ...
      this.width = Math.min(
        ...
        this.wrapper.width() || Infinity,
      ); // Recalculate the width to ignore the affect of borders
      this.paramRegex = this.urlParamTemplate
        ? new RegExp(
    ...
    loadConfig: function () {
      ...
      this.tracks.forEach((track) => {
        ...
        if (
          TrackClass.prototype.id &&
          !tracksById[TrackClass.prototype.id]?._fromStorage
        ) {
          return;
        }
    ...
    saveConfig: function () {
      ...
      this.tracks.forEach((track) => {
        if (
          track.id &&
          !(track instanceof LegendTrack) &&
          !(track instanceof HighlightRegionTrack)
        ) {
    ...
    resetConfig: function () {
      ...
      if (this.tracksById.highlights) {
        ...
        unremovableHighlights = Object.values(
          this.tracksById.highlights.prop("featuresById"),
        );
      }
    ...
    addDomElements: function (width) {
      ...
      this.menus = jQuery();
      this.labelContainer = jQuery('<ul class="gv-label-container">')
        ...
        .sortable({
          ...
          start: function (e, ui) {
            ui.placeholder
              .css({ height: ui.item.height(), visibility: "visible" })
              .html(ui.item.html());
            ui.helper.hide();
          },
        });

      this.wrapper = this.jQuery('<div class="gv-wrapper">').appendTo(
        this.container,
      );
      this.selector = this.jQuery(
        '<div class="gv-selector gv-crosshair">',
      ).appendTo(this.wrapper);

      this.selectorControls =
        this.zoomInHighlight =
        this.zoomOutHighlight =
          jQuery();
      ...
      `).appendTo("body");
        ...
        this.zoomOutHighlight = this.zoomInHighlight
          .clone()
          .toggleClass("gv-i gv-o")
          .appendTo("body");
      }
    },
    ...
    addUserEventHandlers: function () {
      ...
      jQuery(window).on(
        `${this.useHash ? "hashchange" : "popstate"}${this.eventNamespace}`,
    ...
    onTracks: function (func, ...args) {
      this.tracks.forEach((track) => {
        if (track.disabled || !track._interface) {
          // if track._interface is undefined, the track has not been fully initialized yet
          return;
        }
    ...
    mousewheelZoom: function (e, delta) {
      ...
      this.zoomDeltaTimeout = setTimeout(() => {
        if (delta > 0) {
          this.zoomInHighlight
            .css({ left: e.pageX - 20, top: e.pageY - 20, display: "block" })
            .animate(
              {
                width: 80,
                height: 80,
          ...
          this.zoomOutHighlight
            .css({ left: e.pageX - 40, top: e.pageY - 40, display: "block" })
            .animate(
              {
                width: 40,
                height: 40,
      ...
      this.zoomTimeout = setTimeout(() => {
        this[delta > 0 ? "zoomIn" : "zoomOut"](
          e.pageX - this.container.offset().left - this.labelWidth,
        );
    ...
    stopDragSelect: function (e) {
      ...
      const top = Math.min(
        e.pageY - this.wrapper.offset().top,
        this.wrapper.outerHeight(true) -
          1.2 * this.selectorControls.outerHeight(true),
      );
      const pos = this.getSelectorPosition();
      ...
      this.selectorControls
        .find(".gv-selector-location")
        .html(`${this.chr}:${pos.start}-${pos.end}`)
        .end()
        .css({
          top: top,
          left:
            this.selector.outerWidth(true) / 2 -
            this.selectorControls.outerWidth(true) / 2,
        })
        .show();
    },
    ...
    dragSelect: function (e) {
      ...
      if (x > this.selectorStart) {
        this.selector.css({
          ...
          width: Math.min(
            x - this.selectorStart,
            this.width - this.selectorStart - 1,
          ),
        });
      } else {
        this.selector.css({
    ...
    keyup: function (e) {
      if (e.which === 16 && this.prev.dragAction) {
        // shift key
        this.toggleSelect();
      }
    },
    ...
    moveTo: function (chr, start, end, update, keepLength) {
      ...
      if (this.prev.scale === this.scale) {
        this.left = Math.max(
          Math.min(
            this.left + Math.round((this.prev.start - this.start) * this.scale),
            this.maxLeft,
          ),
          this.minLeft,
        );
        this.onTracks(
          ...
          this.start,
          this.end,
          (this.prev.start - this.start) * this.scale,
        );
      }
    },
    ...
    setRange: function (start, end, update, keepLength) {
      ...
      this.start = Math.min(
        Math.max(
          typeof start === "number" ? Math.floor(start) : parseInt(start, 10),
          1,
        ),
        this.chromosomeSize,
      );
      this.end = Math.max(
        Math.min(
          typeof end === "number" ? Math.floor(end) : parseInt(end, 10),
          this.chromosomeSize,
        ),
        1,
      );
      ...
      if (this.end < this.start) {
        this.end = Math.min(
          this.start + this.defaultLength - 1,
          this.chromosomeSize,
        );
      }
      ...
      if (
        update === true &&
        (this.prev.start !== this.start || this.prev.end !== this.end)
      ) {
        this.updateURL();
      }
    },
    ...
    setScale: function () {
      ...
      if (this.prev.scale !== this.scale) {
        this.left = 0;
        this.minLeft = Math.round(
          (this.end - this.chromosomeSize) * this.scale,
        );
        this.maxLeft = Math.round((this.start - 1) * this.scale);
        this.labelBuffer =
          Math.ceil(this.textWidth / this.scale) * this.longestLabel;
    ...
    zoomIn: function (x) {
      ...
      const end =
        this.length === 2 ? start : Math.round(start + (this.length - 1) / 2);
    ...
    zoomOut: function (x) {
      ...
      const end =
        this.length === 1
          ? start + 1
          : Math.round(start + 2 * (this.length - 1));

      this.setRange(start, end, true);
    },

    normaliseTrackDefinition: function (
      trackDefinition,
      baseObjName = "Track",
    ) {
      ...
      const getObj = (t) => {
        ...
        const objName = objNameArray
          .filter((n) => n !== "Genoverse" && !baseObjNameArray.includes(n))
          .join(".");
      ...
      const normalise = (t) => ({
        ...
        ...(t.controller
          ? {
              controller: this.normaliseTrackDefinition(
                t.controller,
                "Track.Controller",
              ),
            }
          : {}),
        ...(t.model
          ? { model: this.normaliseTrackDefinition(t.model, "Track.Model") }
          : {}),
        ...(t.view
          ? { view: this.normaliseTrackDefinition(t.view, "Track.View") }
          : {}),
      });
      ...
      if (
        Array.isArray(trackDefinition) &&
        typeof trackDefinition[0] === "string"
      ) {
        const obj = getObj(trackDefinition[0]);

        return typeof trackDefinition[1] === "object" &&
          Object.keys(trackDefinition[1]).length
          ? obj.extend(normalise(trackDefinition[1]))
          : obj;
      }

      // e.g. { foo: true } -> Genoverse.Track.extend({ foo: true })
      if (
        typeof trackDefinition === "object" &&
        Object.keys(trackDefinition).length
      ) {
    ...
    addTracks: function (tracks, after) {
      ...
      if (push && !this.tracks.filter((t) => typeof t === "function").length) {
        const [insertAfter] = (
          after ? this.tracks.filter((t) => t.order < after) : this.tracks
      ...
      for (let i = 0; i < tracks.length; i++) {
        ...
        tracks[i] = new TrackClass({
          ...
          config: this.savedConfig
            ? jQuery.extend(true, {}, this.savedConfig[TrackClass.prototype.id])
            : undefined,
        });
    ...
    sortTracks: function () {
      ...
      sorted.forEach((track, i) => {
        ...
        const container =
          track.prop("superContainer") || track.prop("container");
      ...
      labels
        .map(function () {
          return jQuery(this).data("track");
        })
        .each(function () {
          if (this.prop("menus").length) {
            const diff =
              (this.prop("superContainer") || this.prop("container")).position()
                .top - this.prop("top");
    ...
    getCoords: function () {
      ...
      let match =
        `${decodeURIComponent(this.useHash ? window.location.hash.replace(/^#/, "?") || window.location.search : window.location.search)}&`.match(
          this.paramRegex,
        );
      let i = 0;
      ...
      this.urlParamTemplate.split("__").forEach((part) => {
        const partMatch = part.match(/^(CHR|START|END)$/);

        if (partMatch) {
          coords[partMatch[1].toLowerCase()] =
            partMatch[1] === "CHR" ? match[i++] : parseInt(match[i++], 10);
        }
      });
      ...
      return coords.chr &&
        coords.start &&
        coords.end &&
        (this.genome ? this.genome[coords.chr] : true)
        ? coords
        : this.initialLocation;
    },
    ...
    getQueryString: function () {
      ...
      let newLocation = currentLocation.match(this.paramRegex)
        ? currentLocation
            .replace(this.paramRegex, `$1${location}$5`)
            .slice(0, -1)
    ...
    getChromosomeSize: function (chr) {
      return chr && this.genome && this.genome[chr]
        ? this.genome[chr].size
        : this.chromosomeSize;
    },
    ...
    makeMenu: function (features, event, track) {
      ...
      const contentEl = jQuery(".gv-menu-content", menuEl).addClass(
        "gv-menu-content-first",
      );
      ...
      (track
        ? track.model.sortFeatures(features)
        ...
        jQuery('<a href="#">')
          .html(title.match(location) ? title : `${location} ${title}`)
          .on("click", (e) => {
            this.makeFeatureMenu(feature, e, track);

            return false;
          })
          .appendTo(jQuery("<td>").appendTo(jQuery("<tr>").appendTo(table)));
      });

      jQuery('<div class="gv-menu-scroll-wrapper">')
        .append(table)
        .appendTo(contentEl);
      ...
      if (event) {
        menuEl
          .css({ left: 0, top: 0 })
          .position({ of: event, my: "left top", collision: "flipfit" });
      }
    ...
    insertPropertiesIntoMenu: function (
      menuProperties,
      menuEl,
      contentEl,
      feature,
      ...
      [].concat(menuProperties).forEach((properties, i) => {
        const el = contentEl
          .clone()
          .addClass(i ? "" : "gv-menu-content-first")
          .appendTo(menuEl);
        const chr =
          typeof properties.chr !== "undefined" ? properties.chr : feature.chr;
        const start = parseInt(
          typeof properties.start !== "undefined"
            ? properties.start
            : feature.start,
          10,
        );
        const end = parseInt(
          typeof properties.end !== "undefined" ? properties.end : feature.end,
          10,
        );
        ...
        jQuery(".gv-title", el)[properties.title ? "html" : "remove"](
          properties.title,
        );
        ...
        if (track && start && end && !this.isStatic) {
          const linkData = {
            ...
            label:
              feature.label || (properties.title || "").replace(/<[^>]+>/g, ""),
            color: feature.color,
          };
        ...
        jQuery("table:not(.gv-focus-highlight)", el)[table ? "html" : "remove"](
          table,
        );
      });
    },
    ...
    makeFeatureMenu: function (feature, e, track) {
      ...
      if (!feature.menuEl || feature.menuEl.data("hasErrored") === true) {
        ...
        const isDeferred =
          typeof getMenu === "object" && typeof getMenu.promise === "function";
        ...
        jQuery
          .when(getMenu)
          .done((menuProperties) => {
            (track?.controller || this).insertPropertiesIntoMenu(
              menuProperties,
              menuEl,
              contentEl,
              feature,
      ...
      if (e) {
        feature.menuEl
          .css({ left: 0, top: 0 })
          .position({ of: e, my: "left top", collision: "flipfit" });
      }
    ...
    on: function (events, obj, fn, once) {
      ...
      Object.entries(eventMap).forEach(([key, value]) => {
        ...
        if (
          !this.events[type][event].some((func) => {
  ...
  },
  {
    ...
    configure: ({
      ...
      if (force || !Genoverse.configured) {
        importTracks(Track, require.context("./Track", true, /\.js$/), {
    ...
    getAllTrackTypes: function (namespace, n) {
      ...
      Object.entries(namespace).forEach(([type, func]) => {
        if (
          typeof func === "function" &&
          !Base[type] &&
          !/^(Controller|Model|View)$/.test(type)
        ) {
    ...
    getTrackNamespace: function (track) {
      ...
      const namespaces = Object.entries(trackTypes)
        .map(
          ([name, constructor]) =>
            track === constructor || track.prototype instanceof constructor
              ? name
              : false, // Find all namespaces which this track could be
        )
        .filter(Boolean);
      ...
      while (namespaces.length > 1) {
        for (let i = 0; i < namespaces.length - 1; i++) {
          if (
            trackTypes[namespaces[i]].prototype instanceof
            trackTypes[namespaces[i + 1]]
          ) {
            namespaces.splice(i + 1, 1);

            break;
          } else if (
            trackTypes[namespaces[i + 1]].prototype instanceof
            trackTypes[namespaces[i]]
          ) {
